...
for i in range(1000):
    # time.sleep(2.2)
    print(json_data)
    res = req.post('http://18.188.111.118:5000/ogscrimmagebotpost/medium',json=swap_elements_in_array(json_data))
    # res = req.post('http://0.0.0.0:5000/ogscrimmagebotpost/easy', json=swap_elements_in_array(json_data))
    print('Header', res.headers['Access-Control-Allow-Origin'])
